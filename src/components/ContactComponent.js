...
class Contact extends Component {
  ...
  validate(firstname, lastname, telnum, email) {
    ...
    const reg = /^\d+$/; // all characters must be numbers

    if(touched.telnum && !reg.test(telnum)) {
      errors.telnum = 'Tel. Number should contain only numbers'
    }

    if(touched.email && email.split('').filter(x => x === 'a').length !== 1) {
  ...
  // render called after input change
  render() {
            ...
            <Form onSubmit={this.handleSubmit}>
              ...
              <FormGroup row>
                ...
                <Col md={{size: 3, offset: 1}}>
                  <Input type="select" name="contactType" value={this.state.conactType} onChange={this.handleInputChange}>
                    <option>Tel.</option>
                    <option>Email</option>
                  </Input>
                </Col>
